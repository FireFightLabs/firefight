...
- **AI SRE records live in the app, the reasoning lives in the engine.** `Investigation` and its nested `Investigation::Hypothesis`, `Investigation::Step` and `Investigation::Finding` are app models, because the dashboard, the API and MCP all read them. `Chat` and `Chat::Message` hold the agent's saved conversation with the model for any owner (an investigation now, a conversation later), on RubyLLM's `acts_as_chat`. `engines/firefight_ai` never names any of them (ArchSpec enforces it): it takes plain input, or the RubyLLM chat a `Chat` wraps, returns plain results, and the app writes the rows. New investigation data joins the `Investigation` family, and anything every agent run needs joins `Chat`, rather than becoming another top-level name.
