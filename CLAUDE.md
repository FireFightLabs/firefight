...
ArchSpec proves the named boundaries. The leaks that hide behind an allowed name (a platform's shape in a column, a permission rule inlined in a controller, logic in an entry point, a TypeScript mirror of a Ruby list) are caught by the **boundary review**: run `/boundary-review` on the diff before opening any PR that touches `app/`, `engines/`, `lib/`, or `app/frontend/`, and fix what it finds. The PR is not ready until it has run. The report does not go in the PR body, and neither does anything the review found and you chose to leave. The repo is public, so a PR never names internal tickets, internal pages or future plans. Tell the person you are working with, and record what was left in the internal tracker.
...
- **A status change two workers could race is one guarded `update_all`, never read then write.** Name the states you are allowed to leave in the `WHERE`, let the row count say whether you won, and `reload` only then. Copy `SolidWorkflow::Workflow#transition!`, `Postmortem.start_generation!` or `Investigation#claim!`. A value that has to survive the race stays in SQL (`started_at: Arel.sql("COALESCE(started_at, now())")`), because a caller holding a stale row would otherwise overwrite it. Rails' own optimistic locking column is not used anywhere here, so do not introduce it for one model.
...
- **AI SRE records live in the app, the reasoning lives in the engine.** `Investigation` and its nested `Investigation::Hypothesis`, `Investigation::Step` and `Investigation::Finding` are app models, because the dashboard, the API and MCP all read them. `Chat` and `Chat::Message` are the agent's saved chat with the model for any owner. `engines/firefight_ai` never names any of them (ArchSpec enforces it): it takes plain input or a `RubyLLM::Chat`, returns plain results, and the app writes the rows. New investigation data joins `Investigation`, data every agent run needs joins `Chat`, never another top-level name. The app side of a run is namespaced the same way: `Investigation::Runner` and `Investigation::Tools::*` live in `app/services/investigation/`, so AI SRE code is `Investigation::*` in the app and `FirefightAi::*` in the engine.
...
- **The operator console is self-contained.** Everything it needs lives under `Operator` (`app/controllers/operator/`, `app/models/operator/`, `app/serializers/operator/`, `app/frontend/pages/operator/`). The console depends on the app, never the other way: no app model, service or job names it, and no app model gains an association or method only the console uses. ArchSpec enforces this. A fact the console shows is something the app records for its own sake (like `PlatformCallFailure`), never a hook added for the console. See docs/operator.md, Boundary.
