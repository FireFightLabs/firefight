class SettingsController < InertiaController
  ...
  def roles
    render inertia: "settings/roles", props: {
      roles: IncidentRoleSerializer.many(current_workspace.incident_roles.ordered.with_usage_counts)
    }
  end
  ...
  def severities
    render inertia: "settings/severities", props: {
      severities: IncidentSeveritySettingsSerializer.many(
        current_workspace.incident_severities.ordered.with_usage_counts
      )
    }
  end
  ...
  def types
    render inertia: "settings/types", props: {
      types: IncidentTypeSettingsSerializer.many(
        current_workspace.incident_types.ordered.with_usage_counts
      )
    }
  end
  ...
  def runbooks
    render inertia: "settings/runbooks", props: {
      runbooks: RunbookSettingsSerializer.many(
        current_workspace.runbooks.ordered.with_usage_counts.includes(:runbook_steps, :incident_conditions)
      ),
  ...
  def forms
    render inertia: "settings/forms", props: {
      ...
      statuses: IncidentStatusSettingsSerializer.many(
        current_workspace.incident_statuses.active.ordered.with_usage_counts
      )
    }
  end
