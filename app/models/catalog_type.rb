class CatalogType < ApplicationRecord
  ...
  def self.create_custom!(workspace:, name:, description: nil, icon: nil, color: nil, attribute_definitions: [])
    transaction do
  ...
  def update_with_definitions!(attrs, attribute_definitions: nil)
    transaction do
      update!(attrs)
      sync_attribute_definitions!(attribute_definitions) if attribute_definitions.present?
      self
    end
  end
  ...
  # Reads the association in memory, so a preloaded type answers without a query.
