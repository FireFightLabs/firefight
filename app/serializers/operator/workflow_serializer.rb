module Operator
  # One workflow run with its steps placed for drawing and every event it recorded.
  class WorkflowSerializer < WorkflowRowSerializer
    object_as :workflow
    ...
    STEP_TYPE = "{ id: string; name: string; status: #{STEP_STATUS_UNION}; attempts: number; maxAttempts: number | null; " \
                ...
                "column: number; row: number; actionBlockedReason: string | null }[]".freeze

    type :string, optional: true
    def pause_reason = workflow.pause_reason

    type :string, optional: true
    def pause_blocked_reason = Actions.pause_blocked_reason(workflow)

    type :string, optional: true
    def resume_blocked_reason = Actions.resume_blocked_reason(workflow)

    type :string, optional: true
    def cancel_blocked_reason = Actions.cancel_blocked_reason(workflow)
    ...
    type STEP_TYPE
    def steps
      WorkflowGraph.new(workflow.steps).nodes.map do |node|
        step = node.step
        {
          ...
          actionBlockedReason: Actions.step_blocked_reason(step)
        }
      end
    end
