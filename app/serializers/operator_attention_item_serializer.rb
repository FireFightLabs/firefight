# One thing on the overview that needs a person, with the page that opens it.
class OperatorAttentionItemSerializer < BaseSerializer
  object_as :item
  ...
  # Jobs open in Flightdeck, which draws its own pages, so the page loads its address whole.
  type :string, optional: true
  def href
