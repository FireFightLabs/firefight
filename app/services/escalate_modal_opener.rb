class EscalateModalOpener
  ...
  def self.cleanup_temp_message(adapter, channel_id, ts)
    ...
    Rails.logger.warn({ event: "escalate_modal_opener.cleanup_temp_failed", error: e.message })
  end
