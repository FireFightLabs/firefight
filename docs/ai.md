...
- **The provider is asked to cache what the agent has read.** One agent holds one growing context and resends all of it every turn, so `Investigator` and `Responder` call `with_caching` on the chat. Like tools and thinking, it is a setting on the in-memory chat, so every job applies it after loading. For Anthropic this is the automatic cache that moves forward with the conversation, and providers that cache on their own are unaffected. A `find_tools` call changes the tool list, which sits in front of everything else in the cached prefix, so each one costs one uncached turn.
...
- **The agent starts with four tools and finds the rest.** `Investigation::Tools.for` hands over `find_tools`, `read_result`, `record_hypothesis` and `conclude`, and nothing else. `find_tools` searches the whole catalog and offers the matches it may use to the live chat, so the next turn calls them for real. Which tools belong in that opening set is a question for the steps a real run records, not a guess.
