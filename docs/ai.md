...
- **A run can offer an incident.** The prompt asks the agent to set `suggest_incident` in `conclude` when there is no incident and users are hurt now, stored as `Finding#suggests_incident` and ignored for a run that has one. The answer then offers Declare incident, in Slack (`Interactions::DeclareFromInvestigationHandler`, the run carried in `ModalState#investigation_id`) and on the dashboard (`investigation_id` on the declare form). `IncidentLifecycleService#create(from_investigation:)` ties the run to the new incident with `Investigation#attach_to!`, once and only from no subject, and the run's start and answer go on the incident's timeline at the times they happened.
