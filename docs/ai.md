...
`/investigations` lists the runs people can see (`Investigation.seen`, so never a rehearsal), newest first and paged, each with its answer or the plain sentence it stopped with. `/investigations/:id` is one run in full: the brief, the finding with each claim linked to the steps it rests on, the theories, and every step with its status, its time, the ledger's decision on it (`Investigation::Step#invocation`) and what it returned, folded. Both are the `investigations` resource's `read`, and both need `Investigation.available_for?`.
...
- **A running run keeps itself current** by reloading only its own prop every few seconds, and stops once it has an answer or has stopped.
- **Every run of an incident is reachable from it.** The incident header links to the run when there is one, and to the list narrowed to the incident (`?incident_id=`) when there are several. The answer and the stopped message in Slack carry an Open in Firefight button to the run's page.
