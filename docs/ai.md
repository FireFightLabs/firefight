...
- **The agent starts with three tools and finds the rest.** `Investigation::Tools.for` hands over `find_tools`, `record_hypothesis` and `conclude`, and nothing else. `find_tools` searches the whole catalog and offers the matches it may use to the live chat, so the next turn calls them for real. Which tools belong in that opening set is a question for the steps a real run records, not a guess.
- **The catalog answers in three ways**, so a gap is explainable rather than invisible: ready to call, exists but not granted to the agent, or a provider nobody connected. Grants still decide, and a search never widens them.
...
- **One turn at a time per conversation.** `ConversationReplyJob` limits concurrency on the conversation id, so a second question asked while the agent is working waits and is answered after the first. RubyLLM saves a blank assistant row while it calls the model, which `Chat#discard_interrupted_reply!` cannot tell from wreckage left by a killed worker, so two turns on one chat end with the live row deleted and its tool results orphaned, which every provider then rejects. The semaphore expires after `TURN_CEILING`, since a turn still holding it by then belongs to a worker that died.
