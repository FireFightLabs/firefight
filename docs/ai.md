...
- **What was running is asked before the agent starts.** `Investigation::RunningCode` runs in the job after the seed pack, once per run. For each service with a repository it calls the GitHub connection's `running_commit` at the time the incident started (`detected_at`, else `declared_at`) through `Chat::Tools::Connection`, the same gateway wrapped tool the agent would call, so the answer is a numbered, ledgered step the finding can cite. The answer is stored under `running_code` in the seed pack, so a resumed run does not ask again. A service with no repository, or a workspace whose GitHub connection has not switched `running_commit` on, gets a sentence saying so rather than a guess. The Investigator prompt says to start from what changed before the running commit and to say whether it came from a deploy record or a guess from the default branch.
- **A service's repository is a catalog role.** `CatalogAttributeDefinition::ROLE_REPOSITORY` marks the text attribute that holds it, on the built-in Service type by default, and `CatalogEntry#repository` reads it as `owner/name` whether it was typed that way or as a web or clone address.
