...
- **The model refuses.** `Incident#lead=` raises `Incident::NotActive` when `lead_assignment_blocked_reason` returns a sentence. Reporting alone is advisory — a caller that forgets to ask is exactly how a resolved incident ends up DMing someone that they are now its lead and rewriting the topic of an archived channel. Refusing catches Slack, the API, MCP, alert ingest, the console, and whatever is written next, from one place. `IncidentLifecycleService#close` sets the lead *before* the status for this reason: the same save closes the incident, and `lead=` refuses once the status has landed.
...
Escalation has no model-level refusal because it has no model-level write — `EscalateIncidentHandler` creates the event and starts the workflow itself, and Slack is its only entry point. Moving it into `IncidentLifecycleService` would put it under the same floor.
