...
- **Where it sits.** "Chat" is the only item under a "Halon" heading at the top of the sidebar. Opening it slides the app sidebar shut (`AuthenticatedLayout sidebarCollapsed`), so the page is two columns: a 16rem chat list on the left and the open chat filling the rest. Below 48rem it is one column at a time, the list or the chat, with an "All chats" link back.
...
- **The story is built outside the markup.** `story.ts` orders the steps, places each theory where it was written down, and tells it again as settled right after the last step it rests on. A theory written down only once settled is told once.
- **Labels are keyed by the generated unions.** Statuses, triggers, theory statuses, step statuses, outcomes and ledger decisions come from `lib/typescript_constants.rb`, and `labels.ts` maps each union to its words with a `Record`, so a new Ruby value fails `tsc` until it has a label. A serialized value is a plain string, so `labelFor` checks it before picking. `tone.ts` gives every status one colour across the view.
- **Steps are anchors.** A claim or a theory lists the steps it rests on as links to `#step-N`, the position the finding cites.
- **A live run polls one prop.** `use-live-investigation.ts` runs `usePoll` with `only` the prop the page names while the run is live, and stops when it is not.
- **Rows are keyed by id.** Evidence and theories ship their ids, since two claims can share words.
