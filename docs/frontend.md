...
- **Where it sits.** "Chat" is the first item under a "Halon" heading at the top of the sidebar, above "Investigations". Opening it slides the app sidebar shut (`AuthenticatedLayout sidebarCollapsed`), so the page is two columns: a 16rem chat list on the left and the open chat filling the rest. Below 48rem it is one column at a time, the list or the chat, with an "All chats" link back.
...
`pages/investigations/` shows Halon's runs, behind the same flag as the chat. `index.tsx` is the paged list, `investigation.tsx` one run. Both are thin shells over `components/`.

- **Labels are keyed by the generated unions.** Statuses, triggers, theory statuses, step statuses, outcomes and ledger decisions come from `lib/typescript_constants.rb`, and `lib/labels.ts` maps each union to its words with a `Record`, so a new Ruby value fails `tsc` until it has a label. A serialized value is a plain string, so `labelFor` checks it before picking.
- **Steps are anchors.** A claim or a theory lists the steps it rests on as links to `#step-N`, the position the finding cites.
- **A live run polls one prop.** `hooks/use-live-investigation.ts` runs `usePoll` with `only` the investigation while its status is live, and stops when it is not.
