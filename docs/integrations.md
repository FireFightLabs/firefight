...
- Outcome contract (`PolicyRule::ApprovalOutcome`): `require: { role, count: 1, self_approval, notify, approvers }`. `approvers` is a list of membership ids validated against the policy's workspace. When present it replaces the role for deciding (`Ability::Approval#approver?`), and the role only describes the request.
