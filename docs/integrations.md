...
- **A pack can connect from a pasted URL.** `connect_with: connection_url` in the registry makes the connect dialog ask for a connection URL per environment (`components/integrations/connection-url-form.tsx`) instead of an install or OAuth. The pack answers `connection_url_refusal` before anything is saved, so a bad or private address is said on the form, and `store_connection_url!` writes it on the environment row, since the pack owns the credential's shape. The same connection name adds an environment or replaces its URL, through `connect!` like OAuth. `connection_url` is filtered from request logs.
