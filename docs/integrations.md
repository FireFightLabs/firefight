...
- **A pack can connect from a pasted URL.** `connect_with: connection_url` in the registry makes the connect dialog ask for a connection URL per environment (`components/integrations/connection-url-form.tsx`) instead of an install or OAuth. The pack answers `connection_refusal` for the URL and any pasted certificates before anything is saved, so a bad or private address or a malformed certificate is said on the form, and `store_connection!` writes them on the environment row, since the pack owns the credential's shape. Each connect sets the whole credential, so certificates left out are removed. The same connection name adds an environment or replaces its URL, through `connect!` like OAuth. `connection_url` and `certificates` are filtered from request logs.
- **PostgreSQL is a native pack** (`Integrations::Packs::Postgres`), connected from a URL by default. The dialog still offers "Use an MCP server instead" for a team that runs one or cannot open its database to the internet, which saves an MCP connection (`kind` is per connection, so both kinds share the entry). Connections made through an MCP server before keep running through MCP. The tools are `list_tables`, `describe_table`, `run_query`, `explain_query`, `current_activity` (connections by state against `max_connections`, the longest running queries, and who blocks whom) and `table_health` (scans, dead rows, vacuum and analyze times), all read only, in the Databases group. Every call opens one connection (`Postgres::Connection`) with `default_transaction_read_only`, a 10 second `statement_timeout`, a lock timeout and an idle timeout, inside `BEGIN READ ONLY`, so a user with write rights still cannot write. `run_query` reads through a cursor, which Postgres only allows for statements that return rows, and fetches at most 200. Statements go through the extended protocol, which takes exactly one, so nothing rides after a semicolon. Cells are cut to 300 characters.
- **A database is only reached on a public address.** The host is resolved before connecting, and a private, loopback, link-local or reserved address (including the 100.64.0.0/10 range clouds use inside their own networks) is refused, since the connection is made from Firefight's network. The connection goes to the resolved address (`hostaddr`), so the name cannot point somewhere else after the check. Sockets and several hosts are refused, and so is any URL option that names a file on Firefight's servers (`sslrootcert` other than `system`, `sslcert`, `sslkey`, `sslcrl`, `passfile`, `service`), since certificates arrive pasted as text. A public database is only reached encrypted: a URL that says nothing or `prefer` gets `require`, and `disable` or `allow` is refused. A pasted CA certificate, client certificate and unencrypted client key are written to files readable by this process only for the length of one connection. An operator allows private hosts with `INTEGRATION_POSTGRESQL_PRIVATE_HOSTS`, comma separated names, addresses or ranges, which is how Firefight reads its own database and how a self-hosted install reaches its own. MCP server addresses have no such check yet.
- **A pack can connect with credentials.** `connect_with: api_token` in the registry makes the connect dialog ask for the fields the pack lists in `credential_fields` (`NativePack::CredentialField`: key, label, hint, placeholder, secret), per environment (`components/integrations/credentials-form.tsx`). The serializer ships the fields, so the form holds no provider list. The pack answers `credential_refusal` by calling the provider with the values before anything is saved, so a wrong token is said on the form, and `store_credentials!` writes them on the environment row. The same name adds an environment or replaces its values. `credentials` is filtered from request logs.
- **Telemetry packs share one shape** (`Integrations::Telemetry`): `LogLine`, `Series` and `Chart` (title, unit, series, range, a link to the provider's live chart). The module writes what the model reads (`logs_text`, `charts_text`, a min, average, max and latest line per series) and the result, which carries charts next to the text as `structuredContent.charts`. A provider pack only maps its API into these shapes. `Chat::Tools::Connection` keeps any charts in a result as `Chat::Chart` rows on the run's or chat's chat, with the tool call id and the run's step number, and hands the model only the text.
- **Northflank is a native telemetry pack** (`Integrations::Packs::Northflank`), connected with a read-only API token and a project id per environment, through `Integrations::NorthflankApi`. The tools are `list_resources` (services and databases with their state), `search_logs` (newest first, at most 200 lines, text, regex, exclude and log type filters), `query_metrics` (CPU, memory, requests, 4xx and 5xx responses, network, TCP connections, disk, returned as numbers and charts) and `recent_builds`, all read only, in the Telemetry group next to the MCP providers. A resource is named by name or id and resolved against the project's list. A chart links to the resource's page in Northflank's app, built from its `appId`, whose first segment is the team. The code sandbox has its own Northflank client, since it runs on Firefight's token and creates services.
- **GitHub is the first native pack** (`Integrations::Packs::Github`): `pr_lookup`, `commit_lookup`, `recent_deployments`, `merged_pull_requests`, `running_commit`, `compare_commits` and `fetch_file` over the REST API, `blame` over GraphQL (the REST API has no blame), and the tools that read code in a sandbox, see Code sandbox below. `fetch_file` and `blame` take a `ref`, so the agent reads the code as it was at the running commit, and every file result carries a link pinned to that commit. `running_commit` returns the last deployment whose own last status is success before a given time, preferring an environment that looks like production, and the one before it to compare against. With no deployment record it falls back to the default branch at that time plus what reached it in the day before, and says in its answer that a merge is not proof of a deploy. `compare_commits` returns the commits, their pull requests with authors and reviewers, the changed files grouped by `CodeChange.kind_for` (migrations, config, feature flags, dependencies, infrastructure, code, tests, docs, in that order), dependency bumps read by `CodeChange::DependencyBumps`, owners from `CODEOWNERS` read by `CodeChange::Owners`, and every diff in the same order. `CodeChange` knows no code host, so a GitLab pack reuses it. `changes_before` (`Github::ChangesBefore`) answers what changed before a time across every repository the installation can see: one commit search per owner for the window, the repositories the clues point at (names, stack frames located through each candidate's tree with a suffix match so a container prefix such as `/app/` still matches, error text through code search), deploys read with the one just before the week so the first has something to compare against, rollbacks named when a deploy is behind the one before, a merge folded into the deploy that shipped it, the ranking from `CodeChange::Ranking`, blame on the failing lines, and a week's summary from each repository's last push. Every search is bounded and every bound it hits, and every search GitHub refuses, is named under Not checked. A deployment counts as succeeded when any of its statuses is success (`GithubApp.deployment_succeeded_at`), since GitHub marks an older deployment inactive once a newer one goes out, and it is timed when that status was written. `list_repositories` lists what the installation can see. `recent_deployments` reads GitHub deployment records, which exist only if the pipeline writes them, and takes the deployment target as `deployment_environment` because `ConnectionToolFactory` already owns the argument name `environment`. `merged_pull_requests` is the fallback when a repo has no deployment records, and it filters closed pull requests down to merged ones client-side because the REST API has no merged-only list. Connect is install-first without OAuth discovery — the customer installs the Firefight GitHub App, the callback brings back an `installation_id` (no tokens), and `Integrations::GithubApp` mints short-lived server-to-server installation tokens from it at call time, cached on the environment row. Requires `INTEGRATION_GITHUB_CLIENT_ID`, `_APP_SLUG`, and `_PRIVATE_KEY` (the App's PEM; the client id doubles as the JWT issuer).

## Code sandbox

The GitHub pack's code tools read in a sandbox, one per run, in `Integrations::Packs::Github::Code`: `list_files`, `code_search`, `find_definition`, `find_usages`, `git_log`, `show_commit`, `diff_refs`, `ask_language_server`, `run_shell` and `run_tests`. Repo content is untrusted input, so the rules are structural, not advisory:

- **One box per run, found by key.** `Chat::Tools::Connection` passes the run's `code_box_key` (an investigation, a conversation, or an outside agent over MCP by principal) through the executor into the pack. `Integrations::CodeReading` finds the live `CodeBox` for the key or starts one, under a Postgres advisory lock so two tools called at once start one box. A code tool called outside a run is refused.
- **The whole history goes in, no credential does.** The first time a run names a repository, the worker mirrors it with the installation token as a per-call `http.extraHeader`, bundles every ref with `git bundle create --all`, and pushes the bundle to the box. The box keeps a bare repository, so any commit reads locally, `git log -S` included. A commit the box has not seen fetches the repository again once. git stderr is scrubbed of the credential before it reaches an error.
- **Commands are argument lists.** The app sends `argv` to the command service in the image (`sandbox/server.rb`), never a string it would have to trust, except `run_shell` and `run_tests`, whose whole point is a command. Refs that start with `-` are refused on both sides. Reads run as `reader`, who cannot write the code. Checkouts are per commit and read only. Tests and language servers run as `runner` in a writable copy per commit.
- **Secrets-shaped paths** are filtered from listings, search and definitions in the pack, as before. `run_shell` can read any file in the checkout, so it and `run_tests` wait for the workspace's AI SRE flag.
- **Language servers** for Ruby, TypeScript and JavaScript, Go and Python run per checkout and language, kept alive for the box's life. The Ruby server runs from its own bundle (`sandbox/lsp/Gemfile`), so it reads a repository's code without installing that repository's gems. Servers that index first are waited on until they report progress done.
- **Tests** install what the repository's lockfiles and version files ask for, once per copy (mise for `.ruby-version`, `.nvmrc`, `.tool-versions`, `mise.toml`, then bundler, npm, yarn, pnpm, pip or go), and can start Postgres and Redis inside the box, which set `DATABASE_URL` and `REDIS_URL`.
- **Providers only start and stop boxes** (`Integrations::Sandboxes`). `SANDBOX_PROVIDER=docker` speaks the Docker Engine API over the daemon socket (`DOCKER_HOST`, default `/var/run/docker.sock`), puts a box on `SANDBOX_DOCKER_NETWORK` when set and reaches it by name, and publishes it on loopback otherwise. `SANDBOX_PROVIDER=northflank` creates a private service in `NORTHFLANK_SANDBOX_PROJECT` with `NORTHFLANK_API_TOKEN`, reached as `http://<service>:8080` inside the project. Both start `SANDBOX_IMAGE`, default the published `ghcr.io/firefightlabs/firefight-sandbox:latest`. Without a provider the tools answer that code reading is not set up. ArchSpec lets only the integrations layer name a provider, so a new one is one class.
- **Boxes stop.** An investigation closes its box when it finishes or gives up. A conversation's box closes after `CodeBox::IDLE_AFTER` without a read (`CodeBoxIdleJob`). `CodeBoxSweepJob`, every 10 minutes, stops boxes unused for `CodeBox::ABANDONED_AFTER` and any box a provider still runs that no row knows, once it is older than the start-up grace.
- **Egress is open.** Northflank's hosted cloud cannot deny outbound traffic from a box, only a BYOC cluster can, and a plain Docker network allows it too. The read tools only run commands the pack builds, so the commands a repository chooses, `run_shell` and `run_tests`, are the ones that wait for the AI SRE flag.
