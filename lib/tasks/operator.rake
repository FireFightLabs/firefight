namespace :operator do
  # For an operator who lost both their phone and their recovery codes. Their next visit to the console sets up anew.
  ...
  task :reset_authenticator, [ :user_id ] => :environment do |_task, args|
    ...
    removed = OperatorCredential.where(user: user).delete_all
