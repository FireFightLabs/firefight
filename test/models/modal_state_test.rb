...
class ModalStateTest < ActiveSupport::TestCase
  ...
  test "encode omits nil temp_message_ts and channel_id rather than serializing nulls" do
    encoded = ModalState.encode(incident_id: 42, temp_message_ts: nil, channel_id: nil)
  ...
  test "parse returns a Result with all fields populated" do
    raw = ModalState.encode(
      incident_id: 42,
      temp_message_ts: "1700000000.000100",
      channel_id: "C12345"
    )

    result = ModalState.parse(raw)
  ...
  test "parse leaves optional fields nil when not encoded" do
    raw = ModalState.encode(incident_id: 42)
  ...
  test "the source message a reaction carries survives the round trip" do
    encoded = ModalState.encode(incident_id: "inc-1", source_message_text: "db is down", source_message_link: "https://slack/p1")
    result = ModalState.parse(encoded)
