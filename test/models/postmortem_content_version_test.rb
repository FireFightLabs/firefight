...
# Replacing the body is the one postmortem write that can throw away somebody else's work,
# so it is the one that asks which version you read.
class PostmortemContentVersionTest < ActiveSupport::TestCase
  ...
  # Status is a separate field, so moving one to in_review while somebody is typing is not a conflict.
