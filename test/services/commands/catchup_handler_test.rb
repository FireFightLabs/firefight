...
class Commands::CatchupHandlerTest < ActiveSupport::TestCase
  ...
  test "enqueues AI response job for active incident" do
    assert_enqueued_with(job: FirefightAi::IncidentResponseJob) do
      result = Commands::CatchupHandler.execute(
  ...
  test "enqueues job with nil thread_ts for channel message" do
    Commands::CatchupHandler.execute(
      build_command(channel_id: @incident.channel_id)
    )
  ...
  test "returns error when workspace not found" do
    ...
    result = Commands::CatchupHandler.execute(command)
