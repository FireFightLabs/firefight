...
class Commands::Firefight::EscalateHandlerTest < ActiveSupport::TestCase
  ...
  test "opens escalate modal in incident channel" do
    stub_post_message
    stub_open_modal

    result = Commands::Firefight::EscalateHandler.execute(
  ...
  test "returns error when workspace not found" do
    command = Command.new(
      ...
      user_id: "U12345678",
      text: Identifiers::SUBCOMMAND_ESCALATE,
      trigger_id: "12345.trigger",
      channel_id: @incident.channel_id,
      metadata: { command: "/ff" }
    )

    result = Commands::Firefight::EscalateHandler.execute(command)
