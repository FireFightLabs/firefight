...
class Commands::TimelineHandlerTest < ActiveSupport::TestCase
  ...
  test "opens timeline modal from incident channel" do
    ...
    result = Commands::TimelineHandler.execute(build_command(channel_id: @incident.channel_id))
  ...
  test "returns error when command is outside incident channel" do
    response = Commands::TimelineHandler.execute(build_command(channel_id: "C_NOT_INCIDENT"))
  ...
  test "returns workspace error when workspace not found" do
    ...
    response = Commands::TimelineHandler.execute(command)
