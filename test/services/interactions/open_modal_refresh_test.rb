...
class Interactions::OpenModalRefreshTest < ActiveSupport::TestCase
  ...
  test "a click from the channel refreshes nothing" do
    ...
    Interactions::OpenModalRefresh.call(interaction(callback_id: nil, view_id: nil), @workspace)
  end
  ...
  test "a click inside the actions modal redraws it" do
    ...
    Interactions::OpenModalRefresh.call(interaction(callback_id: Identifiers::INCIDENT_ACTIONS_MODAL), @workspace)
  ...
  test "an unrecognised modal is left alone rather than guessed at" do
    ...
    Interactions::OpenModalRefresh.call(interaction(callback_id: Identifiers::SET_LEAD_MODAL), @workspace)
  end

  test "a modal whose subject has gone is handled quietly" do
    Slack::Client.expects(:update_modal).never

    assert_nil Interactions::OpenModalRefresh.call(
      interaction(callback_id: Identifiers::INCIDENT_ACTIONS_MODAL, incident_id: SecureRandom.uuid), @workspace
    )
  end

  private

  def interaction(callback_id:, view_id: "V1", incident_id: @incident.id)
    Interaction.new(
