...
class OperatorConsoleTest < ApplicationSystemTestCase
  ...
  test "an operator sets up an authenticator, keeps the recovery codes, and is asked for a code next time" do
    ...
    assert_text OperatorCredential::WRONG_MESSAGE

    fill_in "Code from the app", with: ROTP::TOTP.new(@user.operator_credential.totp_secret).now
    click_button "Confirm and continue"

    assert_text "Save your recovery codes"
    assert_selector "ol li", count: OperatorCredential::RECOVERY_CODE_COUNT
  ...
  test "an operator who set up before is asked for a code, and told a recovery code works too" do
    OperatorCredential.start_for!(@user).confirm!(ROTP::TOTP.new(@user.operator_credential.totp_secret).now)

    visit operator_root_path

    assert_text "Enter your code"
    assert_text "You have #{OperatorCredential::RECOVERY_CODE_COUNT} left."
